...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic is "chicago_kafka_out":  # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic is "kafka_station": # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic is "kafka_turnstile": # Set the conditional to the KSQL Turnstile Summary Topic
